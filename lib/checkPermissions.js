...
function checkPermissionsUsage(files, declaredPermissions) {
    ...
    files.forEach((file) => {
        ...
        // 特殊处理 检查是否使用了 identity.email 权限， https://github.com/Leizhenpeng/check-chrome-perms/issues/1
        if (
          declaredPermissions.has("identity.email") &&
          /chrome\.identity\.getProfileUserInfo\([^)]*\)\s*(?:=>|{)[\s\S]*?(?:email|id)[\s\S]*?(?:\}(?!\s*\));|\);)/.test(content)
        ) {
          unusedPermissions.delete("identity.email");
          usedPermissions.add("identity.email");
        }
